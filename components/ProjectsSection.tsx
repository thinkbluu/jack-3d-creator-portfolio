...
function PortStamp({ year }: { year: string }) {
  ...
  return (
    ...
    >
      <circle cx="32" cy="32" r="28" fill="none" stroke="var(--brass)" strokeDasharray="3 4" strokeOpacity="0.5" />
      <path d="M32 14c.8 4.6 2 6.2 5.5 8-3.5 1.8-4.7 3.4-5.5 8-.8-4.6-2-6.2-5.5-8 3.5-1.8 4.7-3.4 5.5-8Z" fill="var(--brass)" fillOpacity="0.72" />
      <text x="32" y="43" fill="var(--brass)" fillOpacity="0.8" fontSize="9" fontWeight="800" letterSpacing="0.04em" textAnchor="middle">{year}</text>
    </motion.svg>
  )
}
...
function ProjectCard({ project, index }: { project: (typeof projects)[0]; index: number }) {
  ...
  return (
    <div ref={ref} className="flex h-[85vh] items-start justify-center" style={{ paddingTop: `${index * 28}px` }}>
      <motion.article className="sticky top-24 w-full rounded-[28px] border border-[var(--hairline)] bg-[var(--shell-warm)] p-4 md:top-32 md:p-8" style={{ scale, transformOrigin: 'top center' }}>
        <div className="mb-6 flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            <p className="type-kicker text-pretty">ESCALA {project.number} · {project.name} · {project.year}</p>
            <h3 className="type-h3 mt-3 text-[var(--ink)]">{project.name}</h3>
...
export default function ProjectsSection() {
  return (
    <section id="projects" className="section-shell bg-[var(--shell)]">
      <div className="site-container">
        <FadeIn><header className="section-header"><ChartKicker label="Escale" /><h2 className="type-h2">Escale</h2><p className="type-body mt-5">Fiecare proiect e un port în care am lăsat ceva mai bun decât am găsit.</p></header></FadeIn>
