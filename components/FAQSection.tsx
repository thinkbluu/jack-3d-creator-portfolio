...
const FAQItem = memo(function FAQItem({ question, answer, delay }: FAQItemProps) {
  const [isOpen, setIsOpen] = useState(false)
  return (
    <FadeIn delay={delay} y={16}>
      <div className="border-b border-[rgba(245,241,232,0.1)]">
        <button type="button" className="flex w-full cursor-pointer items-center justify-between gap-6 py-6 text-left" aria-expanded={isOpen} onClick={() => setIsOpen((open) => !open)}>
  ...
  return (
    <section className="bg-[#050A14] px-6 py-24">
      <div className="mx-auto max-w-3xl">
        <h2 className="hero-heading mb-14 text-center font-black uppercase leading-none tracking-tight" style={{ fontSize: 'clamp(2rem, 7vw, 90px)' }}>Întrebări frecvente</h2>
